...
const fetchingNotifications = async () => {
  try {
    const response = await axios.get('http://localhost:8000/api/organizer/notification', {
...
const markAsRead = async (id) => {
  try {
    await axios.post(
      'http://localhost:8000/api/organizer/notification/read',
      { notification_id: id },
      {
        headers: { Authorization: `Bearer ${token}` },
      },
...
const markAllAsRead = async () => {
  try {
    await axios.post('http://localhost:8000/api/organizer/notification/read/all',{}, {
      headers: { Authorization: `Bearer ${token}` },
    })
