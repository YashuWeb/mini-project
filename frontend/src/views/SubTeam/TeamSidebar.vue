  ...
  <div>
    ...
    >
      ...
      <div class="flex items-center justify-between h-16 px-6 border-b border-gray-200">
        ...
        <!-- Collapse toggle button (desktop onlEventProy) -->
        <button
      ...
      >
        ...
        <div v-if="!isCollapsed" class="ml-3">
          ...
          <!-- <p class="text-xs text-gray-500">{{currentUser.roles.name}}</p> -->
        </div>
      </div>
    </aside>
  </div>
...
import { ChevronLeft, ChevronRight, Calendar, UsersRound } from 'lucide-vue-next'
...
// Navigation items
const navigation = [
  { name: 'My Events', to: '/teamDashboard', icon: Calendar },
  { name: 'Task Management', to: '/teamDashboard/teamTaskManagement', icon: UsersRound },
  { name: 'Team Management', to: '/teamDashboard/subTeamManagement', icon: UsersRound },
  // { name: 'Settings', to: '/teamDashboard/settings', icon: Settings }
]
</script>
